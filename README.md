...
:**Поиск датасета и его разметка**.

```
Собрать данные → Почистить → Claude размечает ВЫБОРКУ → Обучаем Модель на Выборке-> Модель размечает ВСЕ
```

Human-in-the-Loop проверяет только сомнительные случаи.
...
Шаг 3: AnnotationAgent
        Claude размечает выборку → data/labeled/collected_labeled.parquet
